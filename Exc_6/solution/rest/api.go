...
func GetReceiptFile(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// handle any error!
		out, err := s3.GetObject(r.Context(), "orders", order.GetFilename(), minio.GetObjectOptions{})
		if err != nil {
			slog.Error("Unable to get object from s3", slog.String("error", err.Error()))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to get object from s3")
			return
		}
		...
		_, err = io.Copy(w, out)
		if err != nil {
			slog.Error("Unable to write object to response", slog.String("error", err.Error()))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to write object to response")
			return
		}
	}
}
...
func PostOrder(db *repository.DatabaseHandler, s3 *minio.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Handle errors!
		// store order as Markdown in S3 (MinIO)
		markdownContent := dbOrder.ToMarkdown()
		reader := strings.NewReader(markdownContent)

		// ensure bucket exists
		exists, err := s3.BucketExists(r.Context(), "orders")
		if err != nil {
			slog.Error("Error checking bucket existence", slog.String("error", err.Error()))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to check bucket")
			return
		}
		if !exists {
			err = s3.MakeBucket(r.Context(), "orders", minio.MakeBucketOptions{})
			if err != nil {
				slog.Error("Unable to create bucket", slog.String("error", err.Error()))
				render.Status(r, http.StatusInternalServerError)
				render.JSON(w, r, "Unable to create bucket")
				return
			}
			slog.Info("Created bucket 'orders'")
		}

		// upload the Markdown file
		objectName := dbOrder.GetFilename()
		size := int64(len(markdownContent))
		_, err = s3.PutObject(r.Context(), "orders", objectName, reader, size, minio.PutObjectOptions{ContentType: "text/markdown"})
		if err != nil {
			slog.Error("Unable to put object to S3", slog.String("error", err.Error()))
			render.Status(r, http.StatusInternalServerError)
			render.JSON(w, r, "Unable to store order in S3")
			return
		}

		slog.Info("Uploaded order to S3", slog.String("filename", objectName))
